...
public:
    ...
    void aP() {
        ...
        for (int i = 0; i < 11; i++) {
            cout << nodes[i]->value << " y sus edges: ";
            Node* po = *(&nodes[i]);
            int tamano = po->edges.size();
            for (auto j = po->edges.begin(); j != po->edges.end(); j++)
            {
                cout << (*j)->nodes[0]->value << " <-d:" << (*j)->value[0] << " t:" << (*j)->value[1] << "-->" << (*j)->nodes[1]->value << "       ";
            }
            cout << endl;
        }
    }
    void DFS(Node* startNode)
    {
        ...
        while (!dfsStack.empty()) {
            ...
            for (auto edge : currentNode->edges) {
                Node* neighbor = (edge->nodes[0] == currentNode) ? edge->nodes[1] : edge->nodes[0];
...
//
//    cout << "DFS Result: ";
//    g.DFS(g.GetNode(0));  // Llamada a la función DFS desde el nodo inicial
//
//    cout << "BFS Result: ";
//    g.BFS(g.GetNode(0));  // Llamada a la función BFS desde el nodo inicial
//}
