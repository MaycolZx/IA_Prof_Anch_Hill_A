...
public:
    ...
    //Destruir nodos
    void destruirDeMaycol(float percent)
    {

        int numNodesToChange = int(nodes.size() * nodes[0].size() * percent / 100.0);
    ...
    vector<CNode*> Astar(int startx, int starty, int endx, int endy, int iterationLimit) {
        ...
        struct NodeAstar {
            int x, y, g, h, f; //g = costo desde el inicio, h = distancia restante estimada, f = costo total
        ...
        vector<vector<bool>> closedSet(nodes.size(), vector<bool>(nodes[0].size(), false));
        ...
        //nodo inicial
        NodeAstar starNode{ startx,starty,heuristica(startx,starty,endx,endy),0 };
        openSet.push(starNode);

        //registro de los padres de los nodos
        vector<vector<pair<int, int>>> cameFrom(nodes.size(), vector<pair<int, int>>(nodes[0].size(), { -1,-1 }));

        while (!openSet.empty() && (iterationLimit == -1 || iterations < iterationLimit)) {
            ...
            if (x == endx && y == endy) {
                ...
                reverse(path.begin(), path.end()); // el camino empieza desde el nodo inicial

                for (int i = 0; i < path.size(); i++) {
                    for (int j = 0; j < nodes.size(); j++) {
                        for (int k = 0; k < nodes[0].size(); k++) {
                            if (nodes[j][k]->valuex == path[i].first && nodes[j][k]->valuey == path[i].second) {
            ...
            for (size_t i = 0; i < dirX.size(); i++) {
                ...
                //ver si valen
                if (newx >= 0 && newx < nodes[0].size() && newy >= 0 && newy < nodes.size() && nodes[newy][newx]->estado) {
                    int Gscore = current.g + 10; // costo al sucesor (falta setear a 10 o 10 raiz de 2)

                    //si tiene costo menor y esta en el closedset no entra ya que es un nodo ya visitado
    ...
    vector<CNode*> HillClimbing(CNode* nodo_inicial, CNode* nodo_objetivo)
    {
        ...
        while (!L.empty())
        {
            ...
            for (CEdge* edge : n->edges)
            {
                ...
                if (hijo->estado == false || find(n->camino_desde_inicial.begin(), n->camino_desde_inicial.end(), hijo) != n->camino_desde_inicial.end())
                {
                    continue;
                }
